...
## Application kit packaging revision 2 — 2026-09-21

The build 10 and 11 ZIPs at the existing download URLs were replaced with packaging revision 2. The redundant prelinked `AVIConverter.app/AVIConverter` executable was removed. `AVIConverter.app` now contains resources and frameworks; run `bash relink.sh` to generate the executable locally before signing/installing. The application objects and libraries are unchanged. Verify the updated SHA-256 values before extraction. Both final ZIPs were extracted independently, their manifests verified, and their executables successfully regenerated from the supplied objects. This is a packaging change, not a new app build, device installation test or legal clearance. See [packaging validation](validation/application-kit-packaging-v2.json).
